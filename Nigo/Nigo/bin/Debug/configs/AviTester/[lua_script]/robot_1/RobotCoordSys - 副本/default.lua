...
function onTestAll(self, event, from, to)
    ...
    _gInfoLog(string.format( "part moved in upCam: x: %.2f, y: %.3f, a: %.3f", xOffset, yOffset, aOffset))
    if(math.abs(xOffset) > 3 or math.abs(yOffset) > 3 or math.abs(aOffset) > 0.4) then 
...
function onRecheckPartPos(self, event, from, to)
    ...
    if(math.abs(xOffset) > 0.1 or math.abs(yOffset) > 0.1 or math.abs(aOffset) > 0.5) then -- 只能放宽 目前丢下去有动
